...
class DB {
	 ...
	 * @return	void
	 */
	function connectDB($host = TYPO3_db_host, $user = TYPO3_db_username, $password = TYPO3_db_password,$DataBaseName) {

		if ($this->sql_pconnect($host, $user, $password)) {
			if (!$this->sql_select_db($DataBaseName)) {
	...
	function __construct($host = TYPO3_db_host, $user = TYPO3_db_username, $password = TYPO3_db_password,$conn='',$DataBaseName=''){
		if($conn!=''){
			$this->link=$conn;
			return ;
		}
		$this->connectDB($host, $user , $password,$DataBaseName);
	}
	...
	function debug_check_recordset($res) {
		...
		$msg .= ': function DB->' . $trace[0]['function'] . ' called from file ' .
